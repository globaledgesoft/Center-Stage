...
from .mbn_v3_base import MAX_SIG_SIZE
...
class ElfV3Base(object):
    ...
    def _add_phdr_and_hash_segs(self, sign):
        """
        ...
        hash_segment_size = self._compute_hash_segment_size(sign)
        ...
        if not self.hash_seg_placement or self.hash_seg_placement == 'POST_PHDR_ALIGNED':
            ...
            size_of_hash_seg = self._compute_hash_segment_size(False, False)
    ...
    def _compute_hash_segment_size(self, sign):
    ...
    def _get_sec_parsegen_mbn_properties(self):
        from sectools.features.isc.iot.cfgparser.auto_gen_obj_config import Cfg_Mbn_Properties
